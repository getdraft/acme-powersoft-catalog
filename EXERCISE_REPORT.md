...
| 6 | Priya Natarajan | After the `catalog/` relocation fix, the user reported four browser views still rendering empty (Acceptable Use, Technology Lifecycle, Business Taxonomy, Teams) despite the underlying SDPs and TechnologyComponents now being discoverable. Root cause was two separate governance-layer gaps never authored in Phases 4-5: (1) Capability objects' `implementations[]` registry -- the actual data source for Acceptable Use/Technology Lifecycle per `docs/capabilities.md` -- was never populated via `object_patch` overlays; only the reverse TechnologyComponent-to-capability `capabilities` backref existed (a citation field, not the governance registry), and even that was missing on 8 of 20 TechnologyComponents; (2) `.draft/workspace.yaml`'s `businessTaxonomy` and `vocabulary.teams` were never declared (`vocabulary: {}`), so the Business Taxonomy and Teams views had no data to render even though objects carried ad hoc `businessContext.pillar` strings | Added `capabilities[]` to the 8 TechnologyComponents missing it; authored 15 `object_patch` overlays under `catalog/object-patches/` populating each in-use Capability's `implementations[]` with `ref`/`lifecycleStatus`/`owner.team`; declared `businessTaxonomy.pillars` (4) and `vocabulary.teams` (6) in `workspace.yaml`; also activated the 6 RequirementGroups in `requirements.activeRequirementGroups`, which had been left empty since Phase 4 despite `docs/workspaces.md` stating explicitly that "workspace requirement activation lives in `.draft/workspace.yaml`, not in the presence of YAML files" | **Y** (no schema gap, but a discoverability/governance-onboarding gap: nothing in the tooling flagged that a capability with TechnologyComponents pointing at it had an empty `implementations[]`, or that `vocabulary`/`businessTaxonomy` were undeclared) |
| 6 | Priya Natarajan | While authoring the capability `object_patch` overlays, found two pre-existing copy-paste errors (`tc-azure-service-bus-asset-lease.yaml` and `tc-powerbi-embedded.yaml` both referenced the CI/CD Pipeline capability instead of Messaging/Reporting respectively) and one malformed pre-existing uid (`PV9F22JQ2K-LSE9`, containing the letter "L", outside the valid Crockford-base32 uid charset) | Fixed the two capability refs directly; ran `repair_uids.py --workspace .`, which correctly regenerated the malformed uid and rewrote every referencing field across the workspace, including inside the new `object_patch`'s nested `patch.implementations[].ref` | N (authoring errors, caught by the repair tool working as designed) |
| 6 | Priya Natarajan | Normalizing the new `object_patch` `owner.team` values against `vocabulary.teams` surfaced that shared-service objects (Phase 4, Admin-authored) used full Title Case team names (e.g. `Core Platform Engineering`) while team-specific objects (Phase 5, engineering-authored) used lowercase id slugs (e.g. `core-platform`) for the same `owner.team` field -- `buildTeamMap()` in the generated browser keys teams by the raw string, so this produced 9 Teams-view tiles for 6 real teams | Normalized all `owner.team` values across `catalog/` to the slug form used in `vocabulary.teams.values[].id`, restoring a clean 6-team view | N (process inconsistency between two authoring phases, not a schema gap) |
| 6 | Priya Natarajan | The Deployment Targets browser view flagged "9 ad-hoc targets referenced in SDPs but not declared in the vocabulary." Two were Host uids (the shared AKS node pool, the on-prem Atlanta VM) referenced directly rather than via a vocabulary slug; the other seven were managed-PaaS service slugs (`snowflake`, `azure-sql-database`, etc.) -- the same "PaaS component has no clean home" pattern already flagged elsewhere in this report, here showing up as an undeclared-vocabulary symptom instead of a missing-`host` symptom | Authored `configurations/vocabulary/deployment-targets.yaml` covering all 9 values (with `type`/`provider` classification) and declared `vocabulary.deploymentTargets` in `.draft/workspace.yaml` pointing at it | N (governance/onboarding gap, not a schema gap -- same root cause family as the Acceptable Use/Taxonomy/Teams entry above: workspace-level governance declarations lagging the catalog content that needs them) |
| 6 | Priya Natarajan | While fixing the above, found the 15 `object_patch` overlays from the prior remediation round had been placed under `catalog/object-patches/` instead of `configurations/object-patches/` -- the location `docs/workspaces.md` and `docs/capabilities.md` both document explicitly. It still worked because `object-patches` happens to also be in `generate_browser.py`'s `CATALOG_FOLDERS` allowlist, but it was the wrong convention root (`catalog/` is for generated architecture content; `configurations/` is for company overlays) | Moved the 15 files to `configurations/object-patches/` via `git mv`; verified the browser regenerates identically (144 objects, all 15 capabilities still populated) | N (process error, second occurrence of the same catalog/-vs-configurations/ mistake already logged in Phase 6 above -- worth calling out as a repeat-pattern risk, not a one-off) |
| 6 | Priya Natarajan | Running `validate.py` for the first time with RequirementGroups actually activated (this session's earlier fix) surfaced a large set of pre-existing findings never caught before because activation had been off since Phase 4: (a) 9 TechnologyComponents authored in Phase 5 are missing required fields `vendor`/`productName`/`productVersion`/`classification`, and consequently fail the Vulnerability Management Baseline's VM-01 requirement; (b) ~15 objects across Phases 4-5 carry a deprecated `requirementGroups` field that the framework says is superseded by workspace-level activation; (c) the 6 RequirementGroups' `owner.team: Security & Compliance` was a non-standard value against the newly-declared `vocabulary.teams` list | Fixed the one item this session's own changes caused: added `security-compliance` as a 7th `vocabulary.teams` entry and normalized the RequirementGroups' `owner.team` to match. Left (a) and (b) unfixed and reported here rather than silently absorbed into this round's commit -- they predate this session, are real content-completeness gaps in their own right, and are a large enough remediation to warrant the user's own prioritization call rather than an unprompted sweep | **Y** for (a)/(b) in the sense that nothing surfaced these gaps until validation was actually run end-to-end; the deeper gap is that `requirements.activeRequirementGroups` being empty silently suppressed an entire class of requirement-satisfaction checking for two phases with no warning |
| 7 | Priya Natarajan | The 0.59.0 framework upgrade's `validate.py` flagged all 6 of this workspace's RequirementGroups for being listed in `requirements.activeRequirementGroups` despite each already declaring `activation: always` on the object itself -- a hard error, even though `activation: always` means the group was already being enforced unconditionally and removing the redundant listing produced a byte-identical requirement-satisfaction result (confirmed by diffing `validate.py` output before/after). Root cause traced to this report's own Phase 6 entry above: the activation was added because `docs/workspaces.md` describes activation generically without mentioning the `activation: always` exception, which only lives in the requirement-group schema's field description. Confirmed via `git show` that this exact validator check (and the underlying `activation: always` declarations) already existed pre-0.59.0 -- the bug predates the migration and was simply never surfaced because an earlier validation run was piped through `tail -80` and never displayed the errors section | Removed all 6 uids from `requirements.activeRequirementGroups`, leaving it empty (correct, since none of Acme's RequirementGroups are `activation: workspace` type); verified via diff that requirement-satisfaction results are unchanged | N (the rule itself is arguably miscalibrated -- see Recommended Change #17 -- but the underlying state it flagged was a real authoring mistake from this report's own Phase 6 remediation, not a framework defect) |
...
13. **[High]** Extend the same "warn, don't silently render empty" principle
    from item 11 to the browser's other governance/taxonomy data sources:
    a Capability referenced by a TechnologyComponent's `capabilities[]`
    backref but with an empty `implementations[]`, or a workspace with
    `businessTaxonomy`/`vocabulary.teams` left undeclared, should produce a
    visible warning rather than a quietly empty Acceptable Use, Technology
    Lifecycle, Business Taxonomy, or Teams view. Addresses the Phase 6
    (Priya Natarajan) Friction Log entry on the four empty browser views.
14. **[Low]** Recommend (in onboarding docs or a lint rule) that `owner.team`
    / `definitionOwner.team` values always use the same id-slug form
    declared in `vocabulary.teams`, rather than allowing full display names
    to creep in from one authoring phase and slugs from another. Addresses
    the Phase 6 (Priya Natarajan) Friction Log entry on the 9-vs-6 Teams
    view discrepancy.
15. **[High]** Make `requirements.activeRequirementGroups` being empty
    produce a loud, visible warning (in `validate.py` output and ideally
    the browser) rather than silently skipping all requirement-satisfaction
    checking. This workspace ran two full authoring phases with active
    groups left empty and never knew it; once activated, `validate.py`
    immediately surfaced multiple real, pre-existing completeness gaps
    (missing required TechnologyComponent fields, deprecated
    `requirementGroups` field usage) that had been invisible the entire
    time. Addresses the Phase 6 (Priya Natarajan) Friction Log entry on
    first-time validation findings.
16. **[Medium]** Consider validating `catalog/`-vs-`configurations/`
    placement at lint/validate time -- e.g. flag an `object_patch` file
    found under `catalog/` even though it parses and resolves correctly,
    since the convention is otherwise undetectable until a human reads the
    docs closely. This is the second time in this exercise that authored
    content landed in the technically-working-but-wrong root. Addresses
    the Phase 6 (Priya Natarajan) Friction Log entry on the misplaced
    `object-patches/` directory.
17. **[Low]** Demote the "always-on requirement group listed in
    `requirements.activeRequirementGroups`" check from a blocking error to
    a warning. Confirmed (via before/after diff of `validate.py`'s
    requirement-satisfaction output) that the listing has zero effect on
    which requirements are enforced -- the rule catches a real
    documentation/intent ambiguity (an unconditional baseline control
    appearing to be an opt-in choice) worth flagging, but not a functional
    defect worth hard-failing validation over. Addresses the Phase 7
    Friction Log entry above.
...
| Repository | Contents |
|---|---|
| [getdraft/acme-powersoft-catalog](https://github.com/getdraft/acme-powersoft-catalog) | Vendored DRAFT framework, `catalog/` (6 active RequirementGroups, 18 shared-service objects, 6 teams' product-specific catalog objects, 6 SoftwareDeploymentPatterns), `configurations/` (15 capability-implementation `object_patch` overlays, deployment-targets vocabulary source), `.draft/workspace.yaml` (business taxonomy + teams + deployment-targets vocabulary declared; DRAFT framework 0.59.0), `teams/*/ONBOARDING.md`, generated `docs/` browser (167 objects), this report, `EXERCISE_REPORT_SUPPLEMENT_0590.md` (0.59.0 migration review) |
